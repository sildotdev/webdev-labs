...
const handleServerMessage = ev => {
    ...
    if (data.type === "login") {
        ...
        data.active_users.forEach(user => {
            qs('#users-list').innerHTML += `<li>${user}</li>`;
        });
        qs('#update').innerHTML = `${data.user_joined} has joined the chat.`;
        ...
        // update UI:
        qs('#chat').innerHTML += `<span role="text" class="${data.username === username ? 'right' : 'left'}">${data.username}: ${data.text}</span>`;
    } else {
